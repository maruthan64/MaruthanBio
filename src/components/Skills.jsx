    ...
    <Section id="skills">
      ...
      <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <input
          ...
          className="w-full rounded-lg border border-line bg-surface-raised/50 px-3 py-2.5 font-mono text-xs text-heading placeholder:text-faint focus:border-accent-500/50 focus:outline-none lg:w-72"
        />
      </div>

      {groups.length === 0 ? (
        <Panel className="p-12 text-center">
          <p className="font-mono text-sm text-muted">Nothing matches that filter.</p>
        </Panel>
      ) : (
        <>
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {groups.map((g, i) => (
              <Reveal key={g.title} delay={(i % 3) * 80}>
                <Panel className="h-full p-5 transition-colors hover:border-accent-500/30">
                  <div className="mb-1 font-mono text-[9px] tracking-[0.2em] text-accent-500/70 uppercase">
                    {g.category}
                  </div>
                  <h3 className="mb-4 text-sm leading-snug font-semibold text-heading">{g.title}</h3>
                  <ul className="space-y-2">
                    {g.items.map((item) => (
                      <li
                        key={item}
                        className="flex gap-2.5 font-mono text-[11px] leading-relaxed text-body"
                      >
                        <span className="mt-[3px] h-1 w-1 shrink-0 rounded-full bg-accent-400/70" />
                        <span>{item}</span>
                      </li>
                    ))}
                  </ul>
