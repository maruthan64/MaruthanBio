...
export function Section({ id, children, className = '' }) {
  return (
    <section
      id={id}
      className={`relative z-10 mx-auto w-full max-w-7xl px-5 py-20 sm:px-8 md:py-28 ${className}`}
    >
      {children}
    </section>
  );
...
export function Eyebrow({ children }) {
  return (
    <div className="mb-4 flex items-center gap-3">
      ...
      <span className="font-mono text-[11px] tracking-[0.28em] text-emerald-400 uppercase">
        {children}
      </span>
    </div>
  );
...
export function SectionHeading({ eyebrow, title, blurb }) {
  return (
    <header className="mb-12 max-w-3xl">
      ...
      <h2 className="text-3xl leading-tight font-bold tracking-tight text-slate-50 sm:text-4xl md:text-[2.75rem]">
        {title}
      </h2>
      {blurb ? <p className="mt-5 text-base leading-relaxed text-slate-400">{blurb}</p> : null}
    </header>
  );
}
...
export function Panel({ children, className = '', glow = false }) {
  return (
    <div
      className={`relative rounded-2xl border border-slate-800/80 bg-slate-950/60 backdrop-blur-sm ${
        glow ? 'shadow-[0_0_60px_-25px_rgba(16,185,129,0.5)]' : ''
      } ${className}`}
    >
      {children}
    </div>
...
export function Chip({ children, tone = 'default' }) {
  const tones = {
    default: 'border-slate-700/70 bg-slate-800/50 text-slate-300',
...
export function StatusDot({ tone = 'ok', pulse = true }) {
  const colors = { ok: 'bg-emerald-400', warn: 'bg-amber-400', error: 'bg-rose-500', idle: 'bg-slate-500' };
  return (
    <span
